...
public class EmployeeList extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int numberOfPage = (dao.getNumberOfEmployee() - 1) / pageSize + 1;
            if (pageIndex > numberOfPage) pageIndex=numberOfPage;
            ArrayList<Employee> emps = dao.getAllEmployees((pageIndex-1) * pageSize, pageSize);
            
            request.setAttribute("current", pageIndex);
            request.setAttribute("total", numberOfPage);
            request.setAttribute("controller", "EmployeeList");
